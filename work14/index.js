// 设置秒杀的结束世间为15秒之后
let endseconds = new Date().getTime() + 15 * 10000;
// 声明变量保存剩余的世间
let d = h = m = s = 0;
// 设置定时器，实现限时秒杀的效果
...
function seckill() {
    ...
    if (remaining > 0) {
        ...
        h = parseInt((remaining / 3600) % 24);// 尖酸剩余小时（除以60*60转换为小时，与24取模，获取剩余的小时）
        m = parseInt((remaining / 60) %60);// 计算剩余分钟（除以60转换为分钟，与60取模，获取剩余的分钟）
